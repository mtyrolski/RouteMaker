...
bool caterpillar(Queue* queue, unsigned int* begin, unsigned int* end,
                 long* currentDistance)
{
    if(*end + 1 < queue->size &&
       currentDistance[queue->invertedHeap[*end + 1]] <
           currentDistance[queue->invertedHeap[*end]])
    {
        *end = *end + 1;
    }

    if(currentDistance[queue->invertedHeap[*begin]] >
       currentDistance[queue->invertedHeap[*end]])
    {
...
{
    int i = id;
    int j = (unsigned int)((i - 1) / 2);  // using ints because of looping value
                                          // in case of i == 0

    while(j >= 0)
    {
        if(currentDistance[queue->invertedHeap[i]] >=
           currentDistance[queue->invertedHeap[j]])
        {
            return;
        }
